function initShippingWidget() {
  ...
  const configEl = document.getElementById('product-shipping-progress-config');
  const config = configEl ? JSON.parse(configEl.textContent) : { cartTotal: 0, productPrice: 0 };

  let threshold = parseFloat(widget.getAttribute('data-threshold')) || 590.0;
  let cartTotal = config.cartTotal || 0;
  let productPrice = window.currentPageProductPrice !== undefined ? window.currentPageProductPrice : (config.productPrice || 0);
