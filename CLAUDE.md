...
There is no unit test suite/JS test runner in this repo. "Testing" means `shopify theme check` plus manual verification on the store (see Deployment below) — there is no single-test command to run.
...
This repo pushes directly from GitHub to Shopify — `main` syncs to the theme "TNT/main" on `textilogvoksdug.dk` through Shopify's native GitHub connection, with no build or CI gate in between. **The theme is live on the server but has no customer traffic** (Lars, 2026-08-14): it carries Shopify's MAIN role and is publicly reachable, yet no customers have access to the store, so a broken deploy costs no orders and no customer sees it. A merge to `main` therefore does not need a standing go-ahead — but it *is* immediate, unreviewed and visible on the real store, which is why the one-at-a-time rule below still holds. `.github/workflows/ci.yml` only runs `theme-check` (lint) on push — it does not deploy and passing it is not evidence the page actually renders correctly (see rules below).
...
- **Build in parallel, land one at a time, verify each before the next.** Agents may develop many features concurrently in separate worktrees — that part is fine and fast. But merging to `main` is a queue, not a batch: push ONE feature, wait for the user to confirm it works in a browser, then push the next. Never merge several unverified features in one go.
  ...
  - When something can only be verified by the user (anything visual, anything that needs the real store), say precisely what to open and what to look for, then stop and wait. Don't queue more merges behind an unverified one.
- **Finished work does not sit stranded on a branch.** End a job by merging its feature branch into `main` and pushing to `origin`. Per Deployment above, the store has no customer traffic, so a merge needs no standing go-ahead — but still say what to look at after the merge, and still land one feature at a time (next rule), because attribution is the reason for the queue, not danger. Ask before merging only when the work is genuinely unverified or you have reason to think Lars wants to look first.
...
- **Log every bug in `docs/failure-log.md`.** Whenever a bug is found — reported by the user or spotted by an agent — add an entry in the *same commit as its fix*: mint the id from the clock — run `Get-Date -Format 'yyyyMMdd-HHmmss'` and use `F-<that>`, e.g. `F-20260814-161045`, never a guessed time and never a counter (`F-001`…`F-037` are the old scheme and keep their ids) — insert it at the top of `## Entries`, add a row at the top of the index table, and fill all five fields (Symptom / Root cause / Fix + commit sha / Prevention, plus the date·reach·area line). "Root cause" means the actual mechanism, not a restatement of the symptom. Read the log *before* debugging anything that feels familiar — its `## Recurring patterns` section lists the mistakes this codebase repeats, and it is the reason the file exists, so extend it whenever a new entry is a pattern's second instance. Never delete an entry; if a fix is later reverted or superseded, append an `**Update YYYY-MM-DD:**` line to it.
...
### Primitive theme blocks (`blocks/_heading.liquid`, `_divider.liquid`, `spacer.liquid`, `custom-liquid.liquid`, `text.liquid`)

Layout/text primitives whose filenames and settings ids mirror Shopify's Horizon theme 1:1 (masterplan `docs/superpowers/plans/2026-08-14-primitive-theme-blocks-master.md`), so a Horizon `templates/*.json` block instance drops into TNT and still renders. `_heading` and `text` both drop Horizon's five raw per-instance typography fields (`font`, `font_size`, `line_height`, `letter_spacing`, `case`) — a declared divergence — and instead map Horizon's `type_preset` value to this theme's `.type-*` classes via `snippets/type-preset-class.liquid`, which also maps Horizon's `color` setting value (`var(--color-foreground)` / `var(--color-foreground-heading)` / `var(--color-primary)`) to a text-color class. `_heading` additionally exposes a `source` select (`custom`/`page_title`/`collection_title`/`product_title`) not present in Horizon, replacing the theme's old `blocks/page-title.liquid`.

Block-level padding uses Horizon's logical keys (`padding-block-start/end`, `padding-inline-start/end`); `snippets/block-padding-vars.liquid` reads those first and falls back to the theme's older `padding_top`/`padding_bottom`/`padding_sides` so both conventions coexist in one snippet. `spacer` and `custom-liquid` have no padding settings and don't call it.

Batch 2 adds the media/action primitives: `blocks/button.liquid`, `image.liquid`, `icon.liquid`, `video.liquid`. `button` maps Horizon's `style_class` to `snippets/button.liquid`'s `variant` param, which gained a third `link` variant (plain underlined text) alongside `primary`/`secondary`, plus optional `width`/`background`/`text_color`/`border_color` override params (all inline-style, no `!important`). `icon`'s select lists this theme's own 45 Lucide sprite names (`snippets/icon-sprite.liquid`) rather than Horizon's ~57 product/social icon names, since the two icon sets don't overlap; a stored icon value outside that allowlist (e.g. from pasted Horizon JSON) renders nothing rather than a broken `<use>`. `video` renders uploaded Shopify videos via the new `snippets/video-native.liquid`, and external YouTube/Vimeo URLs via `snippets/video-external.liquid` — a click-to-play facade (cover image + button, no `<iframe>` until clicked) backed by `assets/video-external.js`, which uses a `window`-level guard so its delegated click listener is registered once no matter how many block instances emit the script tag.

Batch 3 adds the commerce primitives: `blocks/social-links.liquid`, `product-card.liquid`, `collection-card.liquid`, `email-signup.liquid`, `menu.liquid`. `social-links` and `product-card`/`collection-card` are flat wrappers with no per-instance color-scheme fields (`background_color`/`border_color`/`text_color`/`inherit_color_scheme`) even though Horizon's real files have some — Horizon drives those via a `color_scheme` picker system this theme doesn't have, so it's a declared, repeated divergence rather than an oversight. `product-card`/`collection-card` render through the existing monolithic `snippets/product-card.liquid`/`collection-card.liquid` (no nested sub-block composition like Horizon's own versions); layout settings (border/radius/width) are applied via block-scoped CSS targeting the snippet's own `.product-card`/`.collection-card__image-wrapper` classes rather than editing the snippets. `email-signup` renders through `snippets/newsletter-form.liquid`'s `'block'` variant, scoped by `block.id` so two instances on one page don't collide on form field ids. `menu` renders a `linklists` menu (one level of submenu) and reuses `blocks/accordion.liquid`'s native `<details>`/`<summary>` pattern for its optional accordion mode — no JS.
...
Single entry point for all icons — a Lucide sprite (`snippets/icon-sprite.liquid`) referenced via `<use>`. Sizing follows font-size (`1em` square) rather than width/height props. `button: true` fixes a clickable icon to a 44×44px touch target (WCAG) via `content-box` padding around a fixed glyph — the glyph is inset from the button's edge by that padding, so a button meant to sit flush against a layout edge needs that padding accounted for (e.g. an offsetting negative margin) if pixel-perfect edge alignment with adjacent non-button content matters. The sprite also carries 13 filled brand marks (Simple Icons, CC0) for `blocks/social-links.liquid`, alongside the 45 stroked Lucide icons — these need the `.icon--brand` class (`fill: currentColor; stroke: none`) instead of `.icon`'s default stroke styling, e.g. `{% render 'icon', name: 'facebook', class: 'icon--brand' %}`.
...
Text colour is three tokens (`--color-text`, `--color-text-muted`, `--color-text-faint`) replacing the nine ad-hoc `text-primary/NN` opacity levels the theme accumulated. `--color-text-muted`/`--color-text-faint` point at `--color-primary` (defined in `snippets/css-variables.liquid`), which `.dark` already swaps, so they need no dark-mode counterpart. `--color-text` itself is defined separately on `body` (`assets/input.css`) from `--color-foreground` — the merchant-facing "Tekstfarve" setting — not from `--color-primary`; a `:root` definition pointing at `--color-primary` was removed 2026-08-14 because `body`'s declaration shadowed it everywhere and the two were silently disagreeing (see `docs/failure-log.md` F-036). A badge is a *surface* (`.type-badge`), not a text style — its content is the same `.type-xs .type-bold .type-caps` label used elsewhere, so 12px exists in exactly one place.
