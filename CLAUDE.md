...
This repo pushes directly from GitHub to Shopify — `main` syncs to the theme "TNT/main" on `textilogvoksdug.dk` through Shopify's native GitHub connection, with no build or CI gate in between. **That store is a development site, not a live customer-facing storefront** (confirmed by Lars, 2026-08-14); the theme may carry Shopify's MAIN role, but no customers see it. A merge to `main` is therefore a deploy to the development store, not a live deploy, and does not need a merge-gate ceremony — but it *is* immediate and unreviewed, which is why the one-at-a-time rule below still holds. `.github/workflows/ci.yml` only runs `theme-check` (lint) on push — it does not deploy and passing it is not evidence the page actually renders correctly (see rules below).
