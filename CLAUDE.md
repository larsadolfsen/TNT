...
Batch 4 unifies the theme's old `group` and `card` blocks into a single `blocks/group.liquid` mirroring Horizon's real `blocks/group.liquid` schema (fetched from theme `190460559693`, which diverges from the batch-4 plan's draft settings list — Horizon uses `content_direction` values `column`/`row`, not `vertical`/`horizontal`, and a *pair* of alignment fields per axis that `visible_if`-swap with direction, not a single `alignment` field). Four fields have no Horizon counterpart, decided in commit `f83e198`: `background_color` is an additive token-based select (`""`=inherit/`transparent`/`light`/`dark`, reusing card's `bg-card-light`/`bg-background-dark` classes) since Horizon's own background system (`inherit_color_scheme`/`color_scheme`, a named-preset system) has no TNT equivalent; `design: raised`'s shadow became an additive `shadow` checkbox (Horizon's group has no shadow setting at all); `card.image_aspect_ratio` was dropped (Horizon's group only supports a background image, no aspect-ratio control, and no migrated instance used it); `group.align_left` was dropped (not in Horizon, unused by any existing instance). `card` itself is deleted once its live template instances are migrated to `group`.
